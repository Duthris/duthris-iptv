...
import { Film, ListVideo, Search } from "lucide-react";
import type { CategoryListItem, PosterListItem } from "@iptv/db";
import { countVodItems, listCategories, listVodItems } from "@iptv/db";
...
export default function MoviesPage() {
  ...
  const consumeMovie = useNavigationStore((state) => state.consumeMovie);
  React.useEffect(() => {
    const pending = consumeMovie();
    ...
    return (
      <AppShell>
        <EmptyState
          ...
          description="Film kütüphanesini görmek için önce bir kaynak ekleyin."
          action={
            <Link href="/playlists">
    ...
    <AppShell bleed>
      <div className="flex h-full flex-col">
        <header className="border-border/70 flex h-14 shrink-0 items-center gap-3 border-b px-4 lg:px-6">
          ...
          <h1 className="text-foreground text-sm font-semibold tracking-tight">Filmler</h1>
        ...
        <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
          ...
          <PosterGrid
            ...
            emptyTitle={query ? "Sonuç bulunamadı" : "Bu kategoride film yok"}
      ...
      <MovieDetail movieId={selectedId} onClose={() => setSelectedId(null)} />
    </AppShell>
  );
}
